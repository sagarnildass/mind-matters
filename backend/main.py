...
async def start_chat_with_session(websocket: WebSocket, user_id: int, background_tasks: BackgroundTasks, session_id: int = None, db: Session = Depends(get_db), redis: aioredis.Redis = Depends(depends_redis)):
    camera_start_requested = False
    await websocket.accept()
    
    session_data, ws_session_data, chat_logs = await initiate_chat(user_id, db, session_id)

    if isinstance(chat_logs, ChatLog):
        chat_logs = [chat_logs]

    # Send chat history if resuming a session
    if session_id and chat_logs:
        print('chat_logs: ', chat_logs)
        # Assuming chat_logs is a list of chat log entries.
        for log in chat_logs:
            await websocket.send_text(f"{log.direction.capitalize()}: {log.content}")
...
async def start_chat_without_session(websocket: WebSocket, user_id: int, background_tasks: BackgroundTasks, db: Session = Depends(get_db), redis: aioredis.Redis = Depends(depends_redis)):
    camera_start_requested = False
    await websocket.accept()
